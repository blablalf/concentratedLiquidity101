...
library Oracle {
        ...
        returns (Observation memory beforeOrAt, Observation memory atOrAfter)
    {
        ...
        while (true) {
            ...
            // update right boundaray because lte(time, beforeOrAt.timestamp, target); == False
            // means that self[i-1 % cardinality] >= target
            if (!targetAtOrAfter) r = i - 1;
            // update left boundaray because lte(time, target, atOrAfter.timestamp) == False
            // means that self[i+1 % cardinality] < target
            else l = i + 1;
        }
    }
